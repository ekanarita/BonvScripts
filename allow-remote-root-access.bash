...
# Prompt for new root password with allowed special characters
until [[ "$newsshpassh" =~ ^[a-zA-Z0-9_@#\$%\^\&\*\(\)]+$ ]]; do
    read -rp " Enter your new Root Password (alphanumeric, underscores, and special characters @_#\$%^&*() only): " -e newsshpassh
    if [[ ! "$newsshpassh" =~ ^[a-zA-Z0-9_@#\$%\^\&\*\(\)]+$ ]]; then
        echo "Password must be alphanumeric with underscores and special characters @_#\$%^&*() only."
    fi
done
